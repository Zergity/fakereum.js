...
export async function rpcClearSandbox(
  ...
  if (!p || typeof p['signature'] !== 'string') {
    return makeError(req.id, ERR_INVALID_PARAMS, 'expected [{include?:address[], exclude?:address[], keepNonzeroNonce?:bool, keepBalances?:bool, signature}]')
  }
  ...
  const keepBalances = p['keepBalances'] === true
  try {
    const digest = clearSandboxDigest(cfg.chainId, include, exclude, keepNonzeroNonce, keepBalances)
    const signer = await recoverEIP712Signer(digest, p['signature'] as Hex)
    if (!isAdmin(cfg, signer)) return makeError(req.id, ERR_SERVER, `signer ${signer} is not an admin`)
    const counts = await clearFn(include, exclude, keepNonzeroNonce, keepBalances)
